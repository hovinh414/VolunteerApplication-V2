...
export class PostController {
  ...
  lockDonationPost = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { donationId, reasonLock } = req.body;
      const orgId  = req.user.userId;
      const donationPostLockResult = await this.postServiceInstance.lockDonationPost(orgId, reasonLock, donationId);
      if(!donationPostLockResult.error)
      {
        return res.status(200).json(ResponseBase(ResponseStatus.SUCCESS, 'Lock success', null));
      }
      else{
        return res.status(500).json(ResponseBase(ResponseStatus.ERROR, donationPostLockResult?.message , null));
      }
    } catch (error) {
      console.error('Error getting posts:', error);
      return res.status(500).json(ResponseBase(ResponseStatus.ERROR, 'Lock fail', null));
    }
  }
