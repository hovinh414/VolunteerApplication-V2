...
export class PostRepository {
    ...
    savePost = async (_post: any) => {
        ...
        if (userResultType.roleId == '656c9bda38d3d6f36ecc8eb6') {
            if (userResultType.isActiveOrganization) {
                ...
                if (postSave.type.toLowerCase() == 'activity') {
                ...
                else if (postSave.type.toLowerCase() == 'donation') {
                    const donationCreate = new Donation({
                        postId: postResult._id,
                    ...
                    const donationResultCreate = await this.donationRepository.createNewDonation(donationCreate);
                    postSave.donationId = donationResultCreate._id;
                    await postSave.save();
                }
                //#endregion POST TYPE
                //#region REDIS CACHE
            ...
            if (cachedPosts && cachedPostidJoined) {
                ...
                const postsInformation = await Promise.all(posts.map(async (post: any) => {
                    ...
                    if (post.type.toLowerCase() === 'activity') {
                        ...
                        postDTO.activityId = post.activityId;
                    } if (post.type.toLowerCase() === 'donation') {
    ...
    getDetailPost = async (_postId: any, _userId: any) => {
        try {
            ...
            const donationResult: any = await this.donationRepository.getDonationById(post?.donationId);
            ...
            if (_userId == '') {
                ...
                if (post.type.toLowerCase() === 'activity') {
                    postDetail.exprirationDate = activityResult.exprirationDate;
                    postDetail.participants = activityResult.participants;
                    postDetail.totalUserJoin = activityResult.totalUserJoin;
                    postDetail.isExprired = activityResult?.isExprired;
                    postDetail.dateActivity = activityResult.dateActivity;
                }
                else if (post.type.toLowerCase() === 'donation') {
                    postDetail.exprirationDate = donationResult.exprirationDate;
                    postDetail.totalUserJoin = donationResult.totalUserJoin;
                    postDetail.isExprired = donationResult.isExprired;
                    postDetail.donateItem = donationResult.donateItem;
                    postDetail.isLock = donationResult?.isLock;
                    postDetail.reasonLock = donationResult?.reasonLock;
                }
                ...
                if (post.type.toLowerCase() === 'activity') {
                    postDetail.exprirationDate = activityResult.exprirationDate;
                    postDetail.participants = activityResult.participants;
                    postDetail.totalUserJoin = activityResult.numOfPeopleParticipated;
                    postDetail.isExprired = activityResult?.isExprired;
                    postDetail.dateActivity = activityResult.dateActivity;
                    const isJoin: any = await this.activityRepository.isJoined(_userId, post.activityId);
                    postDetail.isJoin = isJoin;
                    ...
                    const isJoin: any = await this.donationRepository.isJoined(_userId, post.donationId);
                    postDetail.isJoin = isJoin;

                    const userForCheckType = await this.userRepository.getExistOrgById(_userId);
                    if (userForCheckType?.roleId == '656d7f368f44963447c3ecf7' && isJoin == true) {
                        const isAttendedCheck = await this.donationRepository.isAttended(_userId, post.donationId);
                        postDetail.isAttended = isAttendedCheck
                    }
                    else if (userForCheckType?.roleId == '656c9bda38d3d6f36ecc8eb6' && orgInformationPost._id == _userId) {
                        postDetail.qrcode = donationResult.qrCode;
                        postDetail.isEnableQr = donationResult.isEnableQr;
                    }
    ...
    async attendance(_postId: any, _userId: any) {
        try {
            ...
            const donationIdForJoin = await this.donationRepository.findDonationIdBasePost(_postId);
            const join = await Join.findOne({ donationId: donationIdForJoin, userId: _userId });
    ...
    //org lock donation post when receive enough 
    async lockDonationPost(_orgId: any, _reasonLock: String, _donationId: any) {
        try {
            ...
            if (donation) {
                ...
                if (post?.ownerId == _orgId) {
                    await Donation.findByIdAndUpdate(_donationId, { isLock: true, reasonLock: _reasonLock });
                    return { success: true, data: donation }; // Indicate success with post data
                }
                else {
                    return { error: true, message: 'Unauthorized' };
                }
            } else {
                return { error: true, message: 'donation not found' };
            }
        } catch (error: any) {
            console.error("Error in lockDonationPost:", error);
